...
if($sort != 1) {
    $q = "SELECT image_id,id,image,title,a,category,rating,votes FROM images WHERE approved=1 ORDER BY rating DESC";
} else {
    $q = "SELECT image_id,id,image,title,a,category,rating,votes FROM images WHERE approved=1 ORDER BY image_id DESC";
}
